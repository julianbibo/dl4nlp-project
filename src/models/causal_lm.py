...
####
# this file should not be used if loading models from HuggingFace transformers lib directly
####
...
class NMTModel:
    ...
    def prompt_batch(self, prompts: List[str]) -> List[str]:
        ...
        model_inputs = self.tokenizer(text_batch, return_tensors="pt", padding=True).to(
            self.model.device
        )
