...
Usage:
  python biomed_terms_eval_files.py \
    --corpus_file /path/to/en_de/eng_ger.txt \
    --data_dir /path/to/en_de \
    --src en --tgt de \
    --out_annotations annotations.jsonl \
...
import os, re, json, argparse, time, asyncio, logging, sys, unicodedata
...
def tokens_with_offsets(s): return [(m.group(0), m.start(), m.end()) for m in _token_pat.finditer(s)]
...
def norm_space(s): return re.sub(r"\s+", " ", str(s).strip())
...
def deepseek_extract_pairs_sync(client, src, tgt, src_lang, tgt_lang, retries=3, sleep_s=2.0):
    ...
    for attempt in range(retries):
        try:
            ...
            if RAW_MODEL_LOG:
                append_jsonl(RAW_MODEL_LOG, {"mode":"sync","text":text,"parsed_pairs":pairs,"src":src,"tgt":tgt})
            return pairs
        except Exception as e:
...
def make_async_caller(async_client, limiter, retries, retry_min, retry_max):
    ...
    async def _call(src, tgt, src_lang, tgt_lang):
        ...
        pairs = _parse_pairs(text)
        if RAW_MODEL_LOG:
            append_jsonl(RAW_MODEL_LOG, {"mode":"async","text":text,"parsed_pairs":pairs,"src":src,"tgt":tgt})
        return pairs
    return _call
...
def build_annotation(idx, pid, src, tgt, pairs):
    ...
    for p in pairs:
        ...
        if not s_span or not t_span:
            if DEBUG_REJECTIONS:
                rejected.append({
                    "src_term": s_term, "tgt_term": t_term,
                    "src_error": s_err, "tgt_error": t_err,
                    "src_regex": s_pat, "tgt_regex": t_pat,
                    "src_diag": s_diag, "tgt_diag": t_diag,
                    "src_preview": src[:200], "tgt_preview": tgt[:200],
                })
            continue
        used_src.append(s_span); used_tgt.append(t_span)
    ...
    if DEBUG_REJECTIONS: ann["debug_rejections"] = rejected
    return ann
...
def read_ids(corpus_file, max_examples=None):
    ...
    with open(corpus_file, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if not s: continue
            ids.append(s)
            if max_examples and len(ids) >= max_examples: break
    return ids
...
def extract_and_annotate_files_sync(args):
    ...
    out = []
    for i, (pid, src_txt, tgt_txt, sp, tp) in enumerate(iter_pairs_from_files(ids, args.data_dir, args.src, args.tgt, args.filename_template)):
        if not isinstance(src_txt, str) or not isinstance(tgt_txt, str) or not src_txt.strip() or not tgt_txt.strip():
            log.warning(f"idx={i} missing/empty text for id={pid} (src={sp}, tgt={tp})")
            ann = {"idx": i, "pair_id": pid, "src": norm_space(src_txt), "tgt": norm_space(tgt_txt), "term_pairs": []}
            if DEBUG_REJECTIONS: ann["debug_rejections"] = [{"src_term":"", "tgt_term":"", "src_error":"empty_src_or_tgt"}]
            out.append(ann); continue
        pairs = deepseek_extract_pairs_sync(client, src_txt, tgt_txt, args.src_lang, args.tgt_lang,
                                            retries=args.retries, sleep_s=args.retry_sleep)
        out.append(build_annotation(i, pid, src_txt, tgt_txt, pairs))
    if args.out_annotations: write_jsonl(args.out_annotations, out)
    return out
...
async def extract_and_annotate_files_async(args):
    ...
    ids = read_ids(args.corpus_file, args.max_examples)
    items = list(iter_pairs_from_files(ids, args.data_dir, args.src, args.tgt, args.filename_template))
    limiter = AsyncLimiter(args.rate_limit, time_period=1)
    caller = make_async_caller(
        async_client=aclient, limiter=limiter,
    ...
    async def worker(i, pid, src_txt, tgt_txt, sp, tp):
        async with sem:
            if not isinstance(src_txt, str) or not isinstance(tgt_txt, str) or not src_txt.strip() or not tgt_txt.strip():
                log.warning(f"idx={i} missing/empty text for id={pid} (src={sp}, tgt={tp})")
                rec = {"idx": i, "pair_id": pid, "src": norm_space(src_txt), "tgt": norm_space(tgt_txt), "term_pairs": []}
                if DEBUG_REJECTIONS: rec["debug_rejections"] = [{"src_term":"", "tgt_term":"", "src_error":"empty_src_or_tgt"}]
                results[i] = rec; return
            try:
                pairs = await caller(src_txt, tgt_txt, args.src_lang, args.tgt_lang)
            except Exception as e:
                log.warning(f"idx={i} id={pid} failed: {e!r}"); pairs=[]
            results[i] = build_annotation(i, pid, src_txt, tgt_txt, pairs)
...
def evaluate_predictions(args, annotations=None):
    ...
    for rec in annotations:
        ...
        if em["total"] and em["total"] > 0:
            total_hit += em["matched"]; total_terms += em["total"]
        eval_lines.append({
            "idx": idx, "pair_id": rec.get("pair_id"),
            "prediction": pred, "target_terms": tgt_terms,
            "matched": em["matched"], "total": em["total"],
            "accuracy": em["accuracy"], "details": em["details"],
        })
    summary = {
        "micro_accuracy": (total_hit/total_terms) if total_terms else None,
        "total_terms": total_terms, "total_matched": total_hit,
...
# ---------- CLI ----------
def parse_args():
    p = argparse.ArgumentParser(description="File-based biomedical term extraction + exact-match evaluation")
    # file-based inputs
    p.add_argument("--corpus_file", required=True, help="Text file with one ID per line")
    p.add_argument("--data_dir", required=True, help="Folder containing <id>_<lang>.txt files")
    p.add_argument("--src", required=True, help="Source lang code used in filenames (e.g., en)")
    p.add_argument("--tgt", required=True, help="Target lang code used in filenames (e.g., de)")
    p.add_argument("--filename_template", default="{id}_{lang}.txt",
                   help="Filename template with {id} and {lang} placeholders")

    # model-language hints (for prompt only)
    p.add_argument("--src_lang", default="English", help="Human-readable source language name")
    p.add_argument("--tgt_lang", default="German", help="Human-readable target language name")
    p.add_argument("--max_examples", type=int, default=None)

    # outputs
    p.add_argument("--out_annotations", default="annotations.jsonl")
    p.add_argument("--predictions", default=None, help='JSONL with {"prediction": "..."} per line, aligned to corpus order')
    ...
    # debugging
    p.add_argument("--debug_rejections", action="store_true", help="Include per-example rejection reasons")
    p.add_argument("--raw_model_log", default=None, help="Append raw model outputs here (JSONL)")
